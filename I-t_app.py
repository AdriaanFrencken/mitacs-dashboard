...
with st.sidebar:
    ...
    threshold_input = st.number_input('Pulse Start Threshold (nA)', min_value=1, max_value=10000, value=2000)
...
if uploaded_files:
    ...
    fig.add_hline(y=threshold_input*1e-9, 
                  line_dash="dash", 
                  line_color="grey",
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            title_font=dict(size=20, color='black'),  # Increase axis label font size and set color
            tickfont=dict(size=16, color='black'),    # Increase tick label font size and set color
